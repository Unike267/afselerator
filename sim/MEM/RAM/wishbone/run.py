...
def post_func(results):
    ...
    for key, item in report.tests.items():
        if key == "MEM.tb_ram_wishbone.8.test_ram": # Copy the output csv and the wave in vcd to known path
            copyfile(
                str(Path(item.path) / "log.csv"),
                str(out_dir_csv / "tb_RAM_wishbone_8.csv"),
            )
            copyfile(
                str(Path(item.path) / "ghdl" / "wave.vcd"),
                str(out_dir_wave / "wave_RAM_wishbone_8.vcd"),
            )
        elif key == "MEM.tb_ram_wishbone.16.test_ram": # Copy the output csv and the wave in vcd to known path
            copyfile(
                str(Path(item.path) / "log.csv"),
                str(out_dir_csv / "tb_RAM_wishbone_16.csv"),
            )
            copyfile(
                str(Path(item.path) / "ghdl" / "wave.vcd"),
                str(out_dir_wave / "wave_RAM_wishbone_16.vcd"),
            )
        elif key == "MEM.tb_ram_wishbone.32.test_ram": # Copy the output csv and the wave in vcd to known path
            copyfile(
                str(Path(item.path) / "log.csv"),
                str(out_dir_csv / "tb_RAM_wishbone_32.csv"),
            )
            copyfile(
                str(Path(item.path) / "ghdl" / "wave.vcd"),
                str(out_dir_wave / "wave_RAM_wishbone_32.vcd"),
            )
...
MEM.add_source_files([
    ROOT /                               "tb_RAM_wishbone.vhd",
    ROOT /                              "RAM_wishbone_vcs.vhd",
...
testbench      = MEM.entity("tb_RAM_wishbone")
